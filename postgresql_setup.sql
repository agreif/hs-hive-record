       ...
       if to_regclass('hive_history') is not null then
           if (TG_OP = 'UPDATE' or TG_OP = 'INSERT') then
                insert into hive_history
                       (id, location_id, name, queen_year, description, is_dissolved, version, created_at, created_by, updated_at, updated_by)
                       values
                       (new.id, new.location_id, new.name, new.queen_year, new.description, new.is_dissolved, new.version, new.created_at, new.created_by, new.updated_at, new.updated_by);
                return new;
            end if;
       end if;
       return null; -- result is ignored since this is an after trigger
